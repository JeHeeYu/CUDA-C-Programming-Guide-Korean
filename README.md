...
### 3.2.16.1.2. 메모리 개체 가져오기

### 3.2.16.1.3. 가져온 메모리 개체에 버퍼 매핑

### 3.2.16.1.4. 가져온 메모리 개체에 밉매핑된 배열 매핑

### 3.2.16.1.5. 동기화 개체 가져오기

### 3.2.16.1.6. 가져온 동기화 개체에 대한 신호 보내기/대기
...
### 3.2.16.3.2. 메모리 개체 가져오기

### 3.2.16.3.3. 가져온 메모리 개체에 버퍼 매핑

### 3.2.16.3.4. 가져온 메모리 개체에 밉매핑된 배열 매핑

### 3.2.16.3.5. 동기화 개체 가져오기

### 3.2.16.3.6. 가져온 동기화 개체에 대한 신호 보내기/대기

### 3.2.16.4. Direct3D 11 상호 운용성

### 3.2.16.4.1. 장치 LUIDs 일치

### 3.2.16.4.2. 메모리 개체 가져오기

### 3.2.16.4.3. 가져온 메모리 개체에 버퍼 매핑

### 3.2.16.4.4. 가져온 메모리 개체에 밉매핑된 배열 매핑

### 3.2.16.4.5. 동기화 개체 가져오기

### 3.2.16.4.6. 가져온 동기화 개체에 대한 신호 보내기/대기

### 3.2.16.5. NVIDIA 소프트웨어 통신 인터페이스 상호 운용성(NVSCI, NVIDIA Software Communication Interface Interoperability)

### 3.2.16.5.1. 메모리 개체 가져오기

### 3.2.16.5.2. 가져온 메모리 개체에 버퍼 매핑

### 3.2.16.5.3. 가져온 메모리 개체에 밉매핑된 배열 매핑

### 3.2.16.5.4. 동기화 개체 가져오기

### 3.2.16.5.5. 가져온 동기화 개체에 대한 신호 보내기/대기
